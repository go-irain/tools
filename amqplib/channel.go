...
func NewChannel(amqpuri, exchange, exchangeType, key string, onClose func(*amqp.Error)) (*Channel, error) {
	c := &Channel{
		exchange:     exchange,
	...
	go func() {
		errochan := make(chan *amqp.Error)
		erro := <-conn.NotifyClose(errochan)
		onClose(erro)
	}()
